...
async def run():
    if not prefix:
        return print(f"prefixが設定されていません")
    ...
    try:
        if not os.path.exists("./all_data/user.db"):
            open(f"./all_data/user.db", "w").close()
            async with connect('./all_data/user.db') as conn:
...
class MyBot(commands.Bot):
    def __init__(self, **kwargs):
        ...
        [self.load_extension(f'scripts.{c}') for c in ["command", "debug", "system"]] # scriptsディレクトリからファイルを読み込む
    ...
    async def on_ready(self): # BOTが完全に起動したら通る関数
        try: # ERRORが起きるか起きないか。起きたらexceptに飛ばされる
            ...
            if self.only_admin: # self.only_adminが存在している場合
                return await self.change_presence(activity=Game(name=f"メンテナンス中:sleeping: ", type=1)) # BOTのステータス変更
            else: # 存在してなかった
                return await self.change_presence(activity=Game(name=f"{prefix}help", type=1)) # BOTのステータス変更
            ...
            if user_id in self.ban_member: # ban_memberにユーザーが登録されている場合
                ...
                if message.content.startswith(prefix) and not message.webhook_id and author != self.user or (self.only_admin and user_id in admin_list):
                    if user_id in all_commands_user or channel_id in all_commands_channel: # コマンドがまだ実行中の場合
                        ...
                        async with connect('./all_data/user.db') as conn: # データベースに接続
                            async with conn.cursor() as cur:
